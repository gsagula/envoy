...
protected:
  ...
  void testcompressDecompressWithUncommonParams(
    ...
    decompressor.decompress(accumulation_buffer);
    std::string decompressed_text{accumulation_buffer.toString()};
...
protected:
  ...
  static void unitializedDecompressorTestHelper() {
    ...
    decompressor.decompress(input_buffer);
  }
};
...
TEST_F(ZlibDecompressorImplTest, CompressAndDecompress) {
  ...
  std::string original_text{};
  original_text.append(buffer.toString());
  ASSERT_EQ(151240, buffer.length());

  compressor.compress(buffer, Compressor::State::Finish);
  ASSERT_LE(111000, buffer.length());
